...
- Full-screen navigation now supports live off-route recovery: it detects deviation from the active polyline, reroutes from the rider’s current location, prefers the fastest Google Directions alternative by ETA, and falls back to a destination-only detour if the waypoint chain cannot be routed.
- Navigation instruction UI now shows reroute/detour status text so riders can tell when the app is recovering versus following the normal route.
