...
- **WebSocket ride-room events (2026-04-01)**: `/rides` Socket.IO namespace added to `gateway.ts` with `ride:subscribe`/`ride:unsubscribe` handlers and `emitToRideRoom()` helper. `ride:joined`, `ride:stop_requested`, and `ride:stop_updated` events emitted from `ride.controller.ts` on successful join/requestStop/handleStopRequest. Client `rideSocket.ts` service + `useEffect` in `ride/[id].tsx` subscribes to the room and invalidates ride/stops queries on arrival.
- **Notification push/email job dedup (2026-04-01)**: `enqueueNotificationPush` and `enqueueNotificationEmail` in `jobs.service.ts` now check for existing pending/processing jobs by `notificationId` before enqueuing, preventing duplicate delivery on retry. Retry backoff (30/120/600/1800s) remains centralized in `getRetryDelaySeconds()` and applied uniformly by `worker.ts`.
...
- DEP0169 deprecation mitigation (2026-03-28): moved Swagger spec creation behind lazy `/api-docs` request handling (`server/src/config/swagger.ts` + `server/src/app.ts`) so API startup no longer emits `url.parse()` warning from `swagger-jsdoc` transitive parser libs.
- Empty-state refresh UX fix (2026-03-28): `client/app/(tabs)/feed.tsx` and `client/app/(tabs)/routes.tsx` now always render `FlatList` with `ListEmptyComponent` plus `contentContainerStyle.flexGrow = 1`, preserving pull-to-refresh when there are zero posts/routes.
- Phase 1 live navigation implementation started (2026-03-28): added full-screen route `client/app/ride/[id]/navigation.tsx` with immersive map mode, dark map style, route polyline rendering (Google Directions with fallback), GPS tracking with smooth forward-offset camera follow, top instruction/ETA overlay, and draggable bottom sheet with participant list + host-only end-ride control.
- Navigation module structure added under `client/src/features/navigation/`: `types/navigation.ts`, `services/navigationRouteService.ts`, `components/NavigationInstructionOverlay.tsx`, `components/NavigationBottomSheet.tsx`.
- Ride start flow integration: `client/app/ride/[id].tsx` now redirects leaders to `/ride/:id/navigation` after successful live-session start; active sessions also expose an explicit "Open Full-Screen Navigation" action.
- Router registration updated in `client/app/_layout.tsx` for `ride/[id]/navigation`.
- Navigation runtime stability fix: `client/app/ride/[id]/navigation.tsx` route-loading effect now depends on stable primitive coordinate keys (`startKey`, `endKey`, `waypointKey`) instead of transient object references, preventing `Maximum update depth exceeded` loops during route hydration.
- Navigation bottom-sheet interaction fix (follow-up): switched from translate-based motion to bottom-anchored animated height (`Animated.Value` on `height` with `overflow: hidden`) plus explicit snap threshold and drag velocity logic.
- Navigation bottom-sheet anchoring hardening (2026-03-28): `client/src/features/navigation/components/NavigationBottomSheet.tsx` now uses explicit `style` positioning (`position: absolute; left: 0; right: 0; bottom: 0`) and stacking (`zIndex`/`elevation`) on `Animated.View` rather than class-only absolute utilities, preventing the sheet container from rendering in normal flow at the top on some runtimes. Snap points were also simplified to generic collapsed/expanded ratios without per-device tuning constants.
- Navigation bottom-sheet whitespace fix (2026-03-29): expanded snap height in `client/src/features/navigation/components/NavigationBottomSheet.tsx` is now fully content-measured by combining runtime `onLayout` chrome/header height + `ScrollView` content height, with no extra expansion buffer and generic viewport clamping. This removes large empty space in expanded state without per-device tuning.
- Navigation full-screen exit UX finalized (2026-03-29): standardized on a single top-left back control in `client/app/ride/[id]/navigation.tsx` as the minimize/exit affordance (no bottom-sheet minimize action). The top control overlay now uses elevated stacking (`zIndex`/`elevation`) so it remains visible above instruction overlays.
- Navigation UI de-dup + overlap fix (2026-03-29): removed duplicate ETA/remaining values from `client/src/features/navigation/components/NavigationBottomSheet.tsx` so turn/ETA data is shown only in the top instruction card, and updated `client/src/features/navigation/components/NavigationInstructionOverlay.tsx` to accept `topOffset` so `client/app/ride/[id]/navigation.tsx` can place the card below measured top controls without overlap.
