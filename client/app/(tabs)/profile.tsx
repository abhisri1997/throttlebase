...
export default function ProfileScreen() {
  ...
  const initial = displayRider?.display_name?.charAt(0) || displayRider?.username?.charAt(0) || '?';
  ...
  return (
    <View className="flex-1" style={{ backgroundColor: colors.bg }}>
      <SafeAreaView
        ...
        edges={['top']}
      >
        <Text className="text-xl font-bold" style={{ color: colors.text }}>My Profile</Text>
        <View className="flex-row items-center">
          <TouchableOpacity onPress={() => router.push('/(modals)/settings')} className="p-2 mr-2">
      ...
      <ScrollView className="flex-1">
        ...
        <View className="p-6 items-center" style={{ borderBottomWidth: 1, borderBottomColor: colors.border }}>
          ...
          <Text className="mt-1" style={{ color: colors.textMuted }}>@{displayRider?.username || 'rider'}</Text>
          <Text className="mt-2 text-center" style={{ color: colors.textMuted }}>
            {displayRider?.bio || 'Update your profile to add a bio and tell other riders about yourself.'}
          </Text>
          ...
          >
            ...
            <Text className="font-bold ml-2" style={{ color: colors.text }}>Edit Profile</Text>
          </TouchableOpacity>
          ...
          >
            <View className="items-center">
              <Text className="font-bold text-lg" style={{ color: colors.text }}>
                {displayRider?.stats?.total_rides || displayRider?.total_rides || 0}
              </Text>
              <Text className="text-xs" style={{ color: colors.textMuted }}>Rides</Text>
            </View>
            <View className="items-center">
              <Text className="font-bold text-lg" style={{ color: colors.text }}>
                {displayRider?.stats?.total_distance_km || displayRider?.total_distance_km || 0}km
              </Text>
        ...
        <View className="p-5 mb-10">
          <Text className="text-xl font-bold mb-4" style={{ color: colors.text }}>My Badges</Text>
          {badgesLoading ? (
            ...
            <View className="flex-row flex-wrap">
              {badges.map((b: any, i: number) => (
                ...
                >
                  ...
                  <Text className="text-xs text-center font-bold px-1" style={{ color: colors.text }}>{b.name}</Text>
                </View>
              ))}
            </View>
          ) : (
