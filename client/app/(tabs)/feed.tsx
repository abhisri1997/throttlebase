...
export default function FeedScreen() {
  ...
  const likeMutation = useMutation({
    mutationFn: async (postId: string) => apiClient.post(`/api/community/posts/${postId}/like`),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['feed'] })
  });

  const deletePostMutation = useMutation({
    mutationFn: async (postId: string) => apiClient.delete(`/api/community/posts/${postId}`),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['feed'] })
  });
  ...
  const renderContent = () => {
    ...
    if (isError) {
      return (
        <View className="flex-1 justify-center items-center">
          <Text className="font-bold mb-2" style={{ color: colors.danger }}>Error loading feed</Text>
    ...
    if (!posts || posts.length === 0) {
      return (
        <View className="flex-1 justify-center items-center p-8">
          <Text className="text-center text-lg" style={{ color: colors.textMuted }}>
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <PostCard
            ...
            onEdit={() => router.push({ pathname: '/(modals)/create-post', params: { editId: item.id, defaultContent: item.content } } as any)}
  ...
  return (
    <SafeAreaView className="flex-1 border-t-0" style={{ backgroundColor: colors.bg }} edges={['top']}>
      <View
        className="px-4 py-3"
        style={{ backgroundColor: colors.surface, borderBottomWidth: 1, borderBottomColor: colors.border }}
      >
        <Text className="text-2xl font-bold tracking-tight" style={{ color: colors.text }}>Community Feed</Text>
      </View>
      {renderContent()}
