...
export default function RouteDetailScreen() {
  ...
  const bookmarkMutation = useMutation({
    ...
    onError: (err: any) => {
      Alert.alert('Error', err.response?.data?.error || 'Failed to bookmark route');
    },
  });
  ...
  if (isError || !route) {
    return (
      <SafeAreaView className="flex-1 justify-center items-center" style={{ backgroundColor: colors.bg }}>
        <Text className="font-bold" style={{ color: colors.danger }}>Failed to load route details.</Text>
    ...
    <View className="flex-1" style={{ backgroundColor: colors.bg }}>
      ...
      <View className="h-2/5 w-full relative">
        {mapCoords.length > 0 ? (
          ...
          >
            <Polyline coordinates={mapCoords} strokeColor="#22c55e" strokeWidth={5} />
          ...
          <View className="flex-1 justify-center items-center" style={{ backgroundColor: colors.surface }}>
            <Text style={{ color: colors.textMuted }}>No Map Data Available</Text>
          </View>
        )}
      ...
      <ScrollView className="flex-1">
        <View className="p-5" style={{ borderBottomWidth: 1, borderBottomColor: colors.border }}>
          <Text className="text-3xl font-bold flex-1" style={{ color: colors.text }}>{route.title}</Text>
          <Text className="text-sm mb-4 mt-1" style={{ color: colors.textMuted }}>
            Created by <Text className="font-bold" style={{ color: colors.text }}>{route.creator_name}</Text> • {dateStr}
          </Text>
          <View className="flex-row flex-wrap mt-2">
            <View className="w-1/2 flex-row items-center mb-4">
              <Map color={colors.textMuted} size={18} className="mr-2" />
              <Text style={{ color: colors.textMuted }}>{route.distance_km || 'Unknown'} km Distance</Text>
            </View>
            <View className="w-1/2 flex-row items-center mb-4">
              <Mountain color={colors.textMuted} size={18} className="mr-2" />
              <Text className="capitalize" style={{ color: colors.textMuted }}>{route.difficulty || 'Unrated'} Difficulty</Text>
            </View>
            <View className="w-1/2 flex-row items-center">
              <TrendingUp color={colors.textMuted} size={18} className="mr-2" />
              <Text style={{ color: colors.textMuted }}>+{route.elevation_gain_m || 0}m Elevation</Text>
            </View>
          </View>
        </View>

        <View className="p-5 mb-10">
          <Text className="text-xl font-bold mb-3" style={{ color: colors.text }}>Geographical Profile</Text>
          <Text className="leading-6 mb-4" style={{ color: colors.textMuted }}>
            This route spans a total of {route.distance_km || 0} kilometers and climbs {route.elevation_gain_m || 0} meters.
            It was crowdsourced from {route.creator_name}'s live GPS telemetry.
          </Text>
          <View className="p-4 rounded-xl" style={{ backgroundColor: colors.primary + '1A', borderWidth: 1, borderColor: colors.primary + '4D' }}>
            <Text className="font-bold mb-1" style={{ color: colors.primary }}>Route Status</Text>
            <Text className="text-sm" style={{ color: colors.primary }}>Safe to Ride — Confirmed by community</Text>
          </View>
        </View>
      </ScrollView>
    </View>
