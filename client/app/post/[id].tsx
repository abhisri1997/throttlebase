...
import { View, Text, FlatList, TextInput, TouchableOpacity, ActivityIndicator, KeyboardAvoidingView, Platform, Keyboard, Alert } from 'react-native';
...
export default function PostScreen() {
  ...
  const commentMutation = useMutation({
    mutationFn: async () => {
      if (editingCommentId) {
        await apiClient.patch(`/api/community/comments/${editingCommentId}`, { content: commentText });
      } else {
        await apiClient.post(`/api/community/posts/${id}/comments`, { content: commentText });
      }
    },
    onSuccess: () => {
      setCommentText('');
    ...
    return (
      <SafeAreaView className="flex-1 justify-center items-center" style={{ backgroundColor: colors.bg }}>
        ...
        <TouchableOpacity onPress={() => router.back()} className="mt-4">
          <Text className="font-bold" style={{ color: colors.primary }}>Go Back</Text>
        </TouchableOpacity>
      </SafeAreaView>
    );
  }
  ...
  const renderComment = ({ item }: { item: any }) => {
    ...
    const isEdited = item.updated_at && (new Date(item.updated_at).getTime() - new Date(item.created_at).getTime() > 1000);
    ...
    const handleOptions = () => {
      if (Platform.OS === 'web') {
        ...
        Alert.alert('Comment Options', '', [
          { text: 'Edit', onPress: () => { setEditingCommentId(item.id); setCommentText(item.content); } },
        ...
        <View className="flex-1">
          <View className="flex-row items-center justify-between mb-1">
            <View className="flex-row items-center">
              <Text className="font-bold mr-2" style={{ color: colors.text }}>{item.author_name}</Text>
              <Text className="text-xs" style={{ color: colors.textMuted }}>
                {new Date(item.created_at).toLocaleDateString()}
              </Text>
              {isEdited && (
                <Text className="text-[10px] ml-1 italic" style={{ color: colors.textMuted }}>(edited)</Text>
              )}
            </View>
            {isOwner && (
  ...
  return (
    <SafeAreaView className="flex-1" style={{ backgroundColor: colors.bg }} edges={['top']}>
      ...
      >
        ...
        <Text className="text-2xl font-bold tracking-tight" style={{ color: colors.text }}>Post</Text>
      </View>
      ...
      >
        <FlatList
          ...
          ListHeaderComponent={
            <View className="pt-4 pb-2" style={{ borderBottomWidth: 1, borderBottomColor: colors.border }}>
              <PostCard
                post={post}
                isOwner={post.rider_id === rider?.id}
                onAuthorPress={(riderId) => router.push(`/rider/${riderId}` as any)}
                onLike={() => likeMutation.mutate()}
                onEdit={() => router.push({ pathname: '/(modals)/create-post', params: { editId: post.id, defaultContent: post.content } } as any)}
                onDelete={() => deletePostMutation.mutate()}
              />
              <Text className="px-4 font-bold text-lg mb-2 mt-2" style={{ color: colors.text }}>Comments</Text>
            </View>
          }
          ListEmptyComponent={
        ...
        >
          ...
          <TouchableOpacity
            ...
            style={{ backgroundColor: commentText.trim() ? colors.primary : colors.surface }}
          >
            {commentMutation.isPending ? (
              <ActivityIndicator color="white" size="small" />
            ) : (
              <Send color={commentText.trim() ? 'white' : colors.textMuted} size={20} />
            )}
          </TouchableOpacity>
        </View>
      </KeyboardAvoidingView>
